...
const HealthStatus = () => {
  ...
  const synced = health?.synced ?? false;
      ...
      <div className="flex items-center gap-3 rounded-lg bg-[#0f0f16] p-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-2xl font-semibold text-white">
            {error ? 'Error' : isLoading ? '...' : latestIndexedBlock ?? '—'}
          </p>
          <p className="text-sm text-gray-400">
            Provider: {providerBlock ?? '—'}{' '}
            {latestIndexedBlock && providerBlock
              ? `(Δ ${providerBlock - latestIndexedBlock})`
              : ''}
          </p>
        </div>
        <div className="rounded-full bg-accent/10 px-3 py-1 text-xs text-accent">
          {synced ? 'Healthy' : 'Catching up'}
        </div>
      </div>
    </Card>
  );
