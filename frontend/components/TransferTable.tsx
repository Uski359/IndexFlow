...
const formatValue = (value: string) => {
  ...
  if (num >= 1e18) return `${(num / 1e18).toFixed(3)} IFLOW`;
...
const TransferRow = ({ transfer }: { transfer: Transfer }) => (
  <tr className="border-b border-[#1f1f2a] last:border-0">
    ...
    <td className="px-3 py-3 text-sm text-gray-300">{transfer.blockNumber}</td>
  ...
  const rows =
    isLoading && transfers.length === 0
      ...
      : transfers.map((transfer) => (
          <TransferRow
            key={`${transfer.txHash}-${transfer.blockNumber}-${transfer.from}-${transfer.to}`}
            transfer={transfer}
          />
        ));
